...
wss.on('connection', function(ws) {
  ...
  EventLog.log('Client #%d connected', thisId);

  var sampler = new BandwidthSampler(ws);
  sampler.on('sample', function(bps) {
    cursor.goto(1, 4 + thisId).eraseLine();
    EventLog.log('WebSocket #%d incoming bandwidth: %d MB/s', thisId, Math.round(bps / (1024*1024)));
  });
    ...
    if (!flags.binary) {
      ...
      makePathForFile(currentFile.path, __dirname + '/uploaded', function(error, path) {
        ...
        fs.writeFile(path + '/' + currentFile.name, data, function(error) {
          ++filesReceived;
          // EventLog.log('received %d bytes long file, %s', data.length, currentFile.path);
  ...
  ws.on('close', function() {
    cursor.goto(1, 4 + thisId).eraseLine();
    EventLog.log('Client #%d disconnected. %d files received.', thisId, filesReceived);
  });

  ws.on('error', function(e) {
    cursor.goto(1, 4 + thisId).eraseLine();
    EventLog.log('Client #%d error: %s', thisId, e.message);
  });
});

fs.mkdir(__dirname + '/uploaded', function(error) {
  // ignore errors, most likely means directory exists
  EventLog.log('Uploaded files will be saved to %s/uploaded.', __dirname);
  EventLog.log('Remember to wipe this directory if you upload lots and lots.');
  app.listen(8080);
  EventLog.log('Listening on http://localhost:8080');
});
