...
next = function next(name) {
    EventLog.log(" \x1b[33m" + (Date.now() - cur_start) + "\x1b[0m ms");
    run_next_test();
};
...
tests.EVAL_1 = function () {
    ...
    if (client.server_info.versions[0] >= 2 && client.server_info.versions[1] >= 5) {
        ...
        EventLog.log("Skipping " + name + " because server version isn't new enough.");
        next(name);
    }
};
...
tests.SUBSCRIBE_CLOSE_RESUBSCRIBE = function () {
    ...
    */
    ...
    c2.once("ready", function() {
        EventLog.log("c2 is ready");
        c1.on("ready", function(err, results) {
            EventLog.log("c1 is ready", count);
...
client.once("ready", function start_tests() {
    EventLog.log("Connected to " + client.host + ":" + client.port + ", Redis server version " + client.server_info.redis_version + "\n");
    EventLog.log("Using reply parser " + client.reply_parser.name);
...
client.on("reconnecting", function (params) {
    EventLog.log("reconnecting: " + util.inspect(params));
});
