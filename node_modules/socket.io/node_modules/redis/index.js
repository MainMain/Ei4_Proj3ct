...
RedisClient.prototype.do_auth = function () {
    ...
    if (exports.debug_mode) {
        EventLog.log("Sending auth to " + self.host + ":" + self.port + " id " + self.connection_id);
    }
    ...
    self.send_command("auth", [this.auth_pass], function (err, res) {
        if (err) {
            if (err.toString().match("LOADING")) {
                ...
                EventLog.log("Redis still loading, trying to authenticate later");
                setTimeout(function () {
                    self.do_auth();
        ...
        if (exports.debug_mode) {
            EventLog.log("Auth succeeded " + self.host + ":" + self.port + " id " + self.connection_id);
        }
...
RedisClient.prototype.on_connect = function () {
    if (exports.debug_mode) {
        EventLog.log("Stream connected " + this.host + ":" + this.port + " id " + this.connection_id);
    }
    var self = this;
    ...
    if (this.options.parser) {
        if (! parsers.some(function (parser) {
            if (parser.name === self.options.parser) {
                ...
                if (exports.debug_mode) {
                    EventLog.log("Using parser module: " + self.parser_module.name);
                }
                return true;
            }
        })) {
        ...
        if (exports.debug_mode) {
            EventLog.log("Using default parser module: " + parsers[0].name);
        }
...
RedisClient.prototype.on_info_cmd = function (err, res) {
    ...
    if (!obj.loading || (obj.loading && obj.loading === "0")) {
        if (exports.debug_mode) {
            EventLog.log("Redis server ready.");
        }
        this.on_ready();
    } else {
        ...
        if (exports.debug_mode) {
            EventLog.log("Redis server still loading, trying again in " + retry_time);
        }
        setTimeout(function () {
            self.ready_check();
        }, retry_time);
...
RedisClient.prototype.ready_check = function () {
    ...
    if (exports.debug_mode) {
        EventLog.log("checking server ready state...");
    }
...
RedisClient.prototype.send_offline_queue = function () {
    ...
    while (this.offline_queue.length > 0) {
        command_obj = this.offline_queue.shift();
        if (exports.debug_mode) {
            EventLog.log("Sending offline command: " + command_obj.command);
        }
...
RedisClient.prototype.connection_gone = function (why) {
    ...
    if (exports.debug_mode) {
        EventLog.log("Retry connection in " + this.current_retry_delay + " ms");
    }
    ...
    this.retry_timer = setTimeout(function () {
        if (exports.debug_mode) {
            EventLog.log("Retrying connection...");
        }
...
RedisClient.prototype.on_data = function (data) {
    if (exports.debug_mode) {
        EventLog.log("net read " + this.host + ":" + this.port + " id " + this.connection_id + ": " + data.toString());
    }
...
RedisClient.prototype.return_error = function (err) {
    ...
    if (command_obj && typeof command_obj.callback === "function") {
        ...
        EventLog.log("node_redis: no callback to send error: " + err.message);
...
RedisClient.prototype.return_reply = function (reply) {
    ...
    if (command_obj && !command_obj.sub_command) {
        if (typeof command_obj.callback === "function") {
            ...
            EventLog.log("no callback for reply: " + (reply && reply.toString && reply.toString()));
        }
        ...
        if (Array.isArray(reply)) {
            ...
            if (type === "message") {
                ...
                if (reply[2] === 0) {
                    this.pub_sub_mode = false;
                    if (this.debug_mode) {
                        EventLog.log("All subscriptions removed, exiting pub/sub mode");
                    }
                } else {
...
RedisClient.prototype.send_command = function (command, args, callback) {
    ...
    if ((!this.ready && !this.send_anyway) || !stream.writable) {
        if (exports.debug_mode) {
            if (!stream.writable) {
                EventLog.log("send command: stream is not writeable.");
            }
        }

        if (this.enable_offline_queue) {
            if (exports.debug_mode) {
                EventLog.log("Queueing " + command + " for next server connection.");
            }
    ...
    if (! buffer_args) { // Build up a string and send entire command in one write
        ...
        if (exports.debug_mode) {
            EventLog.log("send " + this.host + ":" + this.port + " id " + this.connection_id + ": " + command_str);
        }
        buffered_writes += !stream.write(command_str);
    } else {
        if (exports.debug_mode) {
            EventLog.log("send command (" + command_str + ") has Buffer arguments");
        }
        buffered_writes += !stream.write(command_str);
        ...
        for (i = 0, il = args.length, arg; i < il; i += 1) {
            ...
            if (Buffer.isBuffer(arg)) {
                if (arg.length === 0) {
                    if (exports.debug_mode) {
                        EventLog.log("send_command: using empty string for 0 length buffer");
                    }
                    ...
                    if (exports.debug_mode) {
                        EventLog.log("send_command: buffer send " + arg.length + " bytes");
                    }
                }
            } else {
                if (exports.debug_mode) {
                    EventLog.log("send_command: string send " + Buffer.byteLength(arg) + " bytes: " + arg);
                }
    ...
    if (exports.debug_mode) {
        EventLog.log("send_command buffered_writes: " + buffered_writes, " should_buffer: " + this.should_buffer);
    }
...
RedisClient.prototype.pub_sub_command = function (command_obj) {
    ...
    if (this.pub_sub_mode === false && exports.debug_mode) {
        EventLog.log("Entering pub/sub mode from " + command_obj.command);
    }
    this.pub_sub_mode = true;
...
RedisClient.prototype.auth = function () {
    ...
    if (exports.debug_mode) {
        EventLog.log("Saving auth as " + this.auth_pass);
    }
