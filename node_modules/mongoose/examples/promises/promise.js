...
mongoose.connect('mongodb://localhost/persons', function (err) {
  ...
  async.each(data, function (item, cb) {
      ...
      // complete
      prom.addBack(function () { EventLog.log("completed"); });

      // add a callback that is only called on complete (success) events
      prom.addCallback(function () { EventLog.log("Successful Completion!"); });

      // add a callback that is only called on err (rejected) events
      prom.addErrback(function () { EventLog.log("Fail Boat"); });
      ...
      prom.then(function (people) {
        ...
        EventLog.log("Oldest person is: %s", oldest);
      }).then(cleanup);
  });
});
