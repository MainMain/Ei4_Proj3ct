 ...
 *       if (err) ..
 *       EventLog.log('there are %d jungle adventures', count);
 *     });
 *
 * @param {Object} conditions
 * @param {Function} [callback]
 ...
 *       EventLog.log('unique urls with more than 100 clicks', result);
 *     })
 *
 ...
 *       if (err) return handleError(err);
 *       EventLog.log('The number of updated documents was %d', numberAffected);
 *       EventLog.log('The raw response from Mongo was ', raw);
 *     });
 *
 * ####Valid options:
 *
 ...
 *       EventLog.log('map reduce took %d ms', stats.processtime)
 ...
 *       , function (err, res) {
 *       if (err) return handleError(err);
 *       EventLog.log(res); // [ { maxBalance: 98000 } ]
 *     });
 *
 ...
 *         if (err) return handleError(err);
 *         EventLog.log(res); // [ { maxBalance: 98 } ]
 *     });
 *
 * ####NOTE:
 *
 ...
 *       EventLog.log(user.weapon.name) // whip
 *     })
 *
 ...
 *         EventLog.log('%s uses a %s', users.name, user.weapon.name)
