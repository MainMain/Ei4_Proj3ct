...
mquery().count(match, function (err, number){
  EventLog.log('we found %d matching documents', number);
})
```
...
base().findOne(function (err, doc) {
  EventLog.log(doc); // { _id: 108, name: 'cajon' })

  base().setOptions({ overwrite: true }).update({ changed: true }, function (err) {
    base.findOne(function (err, doc) {
      EventLog.log(doc); // { _id: 108, changed: true }) - the doc was overwritten
    });
  });
})
```
...
redDrum.count(function (err, n) {
  EventLog.log('there are %d red drums', n);
})
```
...
greatMovies().count(function (err, n) {
  EventLog.log('There are %d great movies', n);
});
