...
function Db(databaseName, serverConfig, options) {
  ...
  if(this.options.safe == null && this.options.w == null && this.options.journal == null && this.options.fsync == null) {
    EventLog.log("========================================================================================");
    EventLog.log("=  Please ensure that you set the default write concern for the database by setting    =");
    EventLog.log("=   one of the options                                                                 =");
    EventLog.log("=                                                                                      =");
    EventLog.log("=     w: (value of > -1 or the string 'majority'), where < 1 means                     =");
    EventLog.log("=        no write acknowledgement                                                       =");
    EventLog.log("=     journal: true/false, wait for flush to journal before acknowledgement             =");
    EventLog.log("=     fsync: true/false, wait for flush to file system before acknowledgement           =");
    EventLog.log("=                                                                                      =");
    EventLog.log("=  For backward compatibility safe is still supported and                              =");
    EventLog.log("=   allows values of [true | false | {j:true} | {w:n, wtimeout:n} | {fsync:true}]      =");
    EventLog.log("=   the default value is false which means the driver receives does not                =");
    EventLog.log("=   return the information of the success/error of the insert/update/remove            =");
    EventLog.log("=                                                                                      =");
    EventLog.log("=   ex: new Db(new Server('localhost', 27017), {safe:false})                           =");
    EventLog.log("=                                                                                      =");
    EventLog.log("=   http://www.mongodb.org/display/DOCS/getLastError+Command                           =");
    EventLog.log("=                                                                                      =");
    EventLog.log("=  The default of no acknowledgement will change in the very near future                =");
    EventLog.log("=                                                                                      =");
    EventLog.log("=  This message will disappear when the default safe is set on the driver Db           =");
    EventLog.log("========================================================================================");
  }
...
Db.prototype.authenticate = function(username, password, options, callback) {
  ...
  if(options.authMechanism == 'MONGODB-CR') {
    ...
    if(hasKerberos == false) {
      EventLog.log("========================================================================================");
      EventLog.log("=  Please make sure that you install the Kerberos library to use GSSAPI                =");
      EventLog.log("=                                                                                      =");
      EventLog.log("=  npm install -g kerberos                                                             =");
      EventLog.log("=                                                                                      =");
      EventLog.log("=  The Kerberos package is not installed by default for simplicities sake              =");
      EventLog.log("=  and needs to be global install                                                      =");
      EventLog.log("========================================================================================");
...
var _executeAuthCreateUserCommand = function(self, username, password, options, callback) {
  ...
  if(roles.length == 0) {
    EventLog.log("Creating a user without roles is deprecated in MongoDB >= 2.6");
  }
