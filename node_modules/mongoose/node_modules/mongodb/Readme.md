  ...
  MongoClient.connect('mongodb://127.0.0.1:27017/test', function(err, db) {
    ...
    collection.insert({a:2}, function(err, docs) {
      ...
      collection.count(function(err, count) {
        EventLog.log(format("count = %s", count));
      });
...
```javascript
  ...
  collection.findOne({_id: new ObjectID(idString)}, EventLog.log)  // ok
  collection.findOne({_id: idString}, EventLog.log)  // wrong! callback gets undefined
```
...
```javascript
  ...
  MongoClient.connect('mongodb://127.0.0.1:27017/test', function(err, db) {
    ...
    db.collection('test').update({hi: 'here'}, {$set: {hi: 'there'}}, {w:1}, function(err) {
      if (err) console.warn(err.message);
      else EventLog.log('successfully updated');
    });
  });
```
